...
- **Web UI (Gradio)**: ブラウザからマイク録音、または **WAV などの音声ファイル** をアップロードして手軽に試せます。
- **結果のダウンロード**: 文字起こしされたテキストをファイル（.txt）としてダウンロードできます。
- **読み仮名（カタカナ）の生成**: **Sudachi** を使用して、文字起こし結果からカタカナ読みを生成し、ダウンロードすることができます。
...
### 操作方法
1.  **Audio Input**: マイクで録音するか、音声ファイル（WAV, MP3等）をアップロードします。
2.  **Select Model**: 試したいモデルを選択します。
    *   初回選択時はモデルのダウンロードが行われるため、完了まで時間がかかります。
    *   GPU メモリ（VRAM）が小さい環境では、Large モデルや Phi-4 は動作しない場合があります。
3.  **Transcribe**: ボタンを押すと音声認識が実行され、結果がテキストボックスに表示されます。
4.  **Download**: 
    *   「Download Text」：通常のテキストを保存します。
    *   「Download Reading (Katakana)」：Sudachiで変換したカタカナ読みを保存します。
